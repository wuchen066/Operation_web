<template>
  <div class="min-h-screen flex flex-col">
    ...
    <main class="flex-1 flex overflow-hidden">
      <!-- 侧边栏导航 -->
      <aside :class="['bg-white shadow-md h-full flex-shrink-0 overflow-y-auto transition-all duration-300', isSidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full w-0 md:translate-x-0 md:w-64']" id="sidebar">
        <nav class="py-4">
          <div class="px-4 mb-4">
          ...
          <div class="space-y-1 px-2">
            ...
            <router-link to="/system" :class="['flex items-center px-4 py-3 rounded-lg transition-colors', $route.path === '/system' ? 'text-primary bg-primary/10' : 'text-gray-600 hover:bg-gray-100']">
              <i class="fa fa-desktop w-5 text-center mr-3"></i>
              <span>系统监控</span>
            </router-link>
            <router-link to="/commands" :class="['flex items-center px-4 py-3 rounded-lg transition-colors', $route.path === '/commands' ? 'text-primary bg-primary/10' : 'text-gray-600 hover:bg-gray-100']">
              <i class="fa fa-terminal w-5 text-center mr-3"></i>
              <span>命令执行</span>
            </router-link>
            <router-link to="/settings" :class="['flex items-center px-4 py-3 rounded-lg transition-colors', $route.path === '/settings' ? 'text-primary bg-primary/10' : 'text-gray-600 hover:bg-gray-100']">
      ...
      <!-- 内容区域 -->
      <div class="flex-1 overflow-y-auto bg-gray-50 p-4" id="content">
        <router-view />
      </div>
    </main>
