...
function parse(source, compiler = {
    ...
    const ast = compiler.parse(source, {
        ...
        getTextMode: ({ tag, props }, parent) => {
            ...
            // containers
            if ((parent == null && tag !== 'template') ||
                // <template lang="xxx"> should also be treated as raw text
                (tag === 'template' &&
                    props.some((p) => p.type === templateAstTypes.NodeTypes.ATTRIBUTE &&
                        p.name === 'lang' &&
                        p.value != null &&
    ...
    if (descriptor.template != null && !isParsingBrokenTemplate) {
        ...
        if (end < descriptor.template.loc.end.offset) {
            ...
            const proxy = source.slice(0, start) +
...
function createBlock(node, source) {
    ...
    if (node.children.length > 0) {
        start = shared.first(node.children).loc.start;
        end = shared.last(node.children).loc.end;
    ...
    node.props.forEach((p) => {
        if (p.type === templateAstTypes.NodeTypes.ATTRIBUTE) {
            attrs[p.name] =
