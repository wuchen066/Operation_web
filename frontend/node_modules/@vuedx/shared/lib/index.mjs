...
function getComponentName(fileName) {
    const name = pascalCase(Path.posix
    ...
    return (target, key, descriptor) => {
        ...
        descriptor.value = function (...args) {
            if (isDebugging && !printOnlyReturnStatement) {
                this.logger.debug(`(call) ${target.constructor.name}.${String(key)}`, inspect(transformArgs(...args), false, 100, false));
            }
            const result = fn.apply(this, args);
            if (isDebugging) {
                this.logger.debug(`(return) ${target.constructor.name}.${String(key)}`, inspect(transformArgs(...args), false, 100, false), inspect(transformReturn(result), false, 100, false));
            }
            return result;
        };
        return descriptor;
...
export { BinarySearchBias, DisposableScope, Event, EventEmitter, HTML_TAG_NAME_TO_CLASS_NAME, index as RPC, SVG_TAG_NAME_TO_CLASS_NAME, SetOps, SourceTransformer, Telemetry, binarySearch, binarySearchKey, cache, cacheAll, cacheAllAsync, cacheAsync, camelCase, camelize, capitalize, chunk, clearMethodCache, collect, collectError, computed, concat, createCache, createModel, createMultiKeyCache, createVersionedCache, createWeakMapCache, debug, effect, findNextSibling, findPrevSibling, first, flatten, generateUUID, getClassNameForTagName, getComponentName, getComponentNameAliases, getMappings, getRelativeFileName, hyphenate, invariant, isArray, isCamelCase, isFilesystemSchemeFile, isHTMLTag, isKebabCase, isNotNull, isNumber, isObject, isPascalCase, isPlainObject, isProjectRuntimeFile, isRawTextContainer, isSVGTag, isString, isVoidTag, isVueFile, isVueJsxFile, isVueRuntimeFile, isVueSFCDescriptorFile, isVueTemplateASTFile, isVueTsxFile, isVueVirtualFile, kebabCase, last, lcfirst, mayContainVirtualFileName, measure, memoize, memoizeByFirstArg, parseFileName, pascalCase, readonly, rebaseSourceMap, setDebugging, startMeasure, toFileName, toPlatformPath, toPosixPath, toWindowsPath, trace, tracePromise, trimIndent, ucfirst, uncapitalize, versioned, versionedAsync };
