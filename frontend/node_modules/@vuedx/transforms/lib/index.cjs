...
function findIdentifiers(ts, program, sourceFile) {
    ...
    checker
        ...
        if (kind === 'externalMaybeRef' &&
            ...
            const declaration = shared.first(sym.declarations);
...
function transformScript(source, options) {
    ...
    shared.invariant(sourceFile != null, 'Source file not found.');
    let defaultExport;
    let inheritAttrs = true;
    let name = shared.getComponentName(options.fileName);
    const vars = {
    ...
    const code = new shared.SourceTransformer(inputFile, source);
    ...
    function findNodes(sourceFile) {
        ...
        function processProps(node) {
            if (node.typeArguments != null && node.typeArguments.length > 0) {
                propsType = shared.first(node.typeArguments);
            }
            else if (node.arguments != null && node.arguments.length > 0) {
                propsOptions = shared.first(node.arguments);
            }
        }
        function processEmits(node) {
            if (node.typeArguments != null && node.typeArguments.length > 0) {
                emitsType = shared.first(node.typeArguments);
            }
            else if (node.arguments != null && node.arguments.length > 0) {
                emitsOptions = shared.first(node.arguments);
            }
        }
        function processExpose(node) {
            if (node.arguments != null && node.arguments.length > 0) {
                exposeOptions = shared.first(node.arguments);
            }
        }
    }
}
